...
let progress={map:false,dash:false,book:false,page:false,spell:false,thorns:false,boss:false,visited:[0]},checkpoint={room:0,x:100,y:420};
...
const rooms=[
 {name:'El umbral verde',kind:'forest',platforms:[[0,480,960,60],[230,398,130,16],[420,326,130,16],[620,256,140,16],[810,192,150,16],[820,105,140,16],[820,20,140,16]],exits:{right:1,up:3},spawn:[[550,448,'goblin']],items:[['shrine',100,480],['map',175,480]],hint:'A / D: moverse · Espacio: saltar · J: espada · E: interactuar.'},
...
function help(){if(!['play','title'].includes(mode))return;const wasTitle=mode==='title';mode='pause';panel('<div class="eyebrow">UN RESPIRO ENTRE LAS RAÍCES</div><h2>El bosque puede esperar</h2><p>A / D o flechas · Moverse<br>Espacio / W / ↑ · Saltar &nbsp; J · Espada<br>Shift · Impulso (requiere botas)<br>K · Ascua (requiere descifrar la página)<br>E · Interactuar &nbsp; Tab · Grimorio &nbsp; M · Mapa &nbsp; Esc · Pausa<br><br>La espada restaura magia al acertar. El impulso protege un instante. Los santuarios curan y guardan. Las salidas doradas conectan las salas.</p><button class="primary" id="back">VOLVER</button><div class="menu-options"><button class="secondary" id="sound"></button><button class="secondary" id="fullscreen">F · PANTALLA COMPLETA</button></div>');bindMenu();$('#back').onclick=wasTitle?title:resume;}
window.addEventListener('keydown',e=>{if(e.code==='KeyM'&&['play','map'].includes(mode)){e.preventDefault();if(!e.repeat){if(mode==='map')resume();else openMap();}return;}if(e.code==='KeyF'){e.preventDefault();if(!e.repeat)toggleFullscreen();return;}if(mode!=='play'&&['ArrowUp','ArrowDown','ArrowLeft','ArrowRight'].includes(e.code)){e.preventDefault();const buttons=[...document.querySelectorAll('#overlay button')];const i=buttons.indexOf(document.activeElement),d=['ArrowUp','ArrowLeft'].includes(e.code)?-1:1;buttons[(i+d+buttons.length)%buttons.length]?.focus();return;}if(['Space','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Tab'].includes(e.code)&&mode==='play')e.preventDefault();if(e.code==='Escape'){if(mode==='play')help();else if(['journal','cipher','map'].includes(mode))resume();else if(mode==='pause')$('#back').click();return;}if(e.code==='Tab'&&mode==='play'){journal();return;}if(!keys.has(e.code))pressed.add(e.code);keys.add(e.code);});window.addEventListener('keyup',e=>keys.delete(e.code));window.addEventListener('blur',()=>{keys.clear();pressed.clear();if(mode==='play')help();});
...
function interact(){for(const [type,x,y]of rooms[room].items){if(Math.hypot(p.x+9-x,p.y+32-y)>65)continue;if(type==='shrine'){checkpoint={room,x:x-9,y:y-32};p.hp=5;p.mana=3;resetEncounters();enter(room,p.x,p.y);p.inv=1.25;save();toast('Santuario encendido · Progreso guardado · Salud y magia restauradas');beep(600,.3);}if(type==='map'&&!progress.map){progress.map=true;save();toast('MAPA RECUPERADO · M para desplegarlo.');beep(580,.2);}if(type==='dash'&&!progress.dash){progress.dash=true;save();toast('BOTAS DEL VIENTO · Shift para impulsarte, incluso en el aire.');beep(650,.3);}if(type==='book'&&!progress.book){progress.book=true;save();toast('GRIMORIO ANTIGUO · Buscá las páginas perdidas. Tab para consultar tus hallazgos.');}if(type==='page'&&!progress.page){progress.page=true;save();toast('PÁGINA ENCONTRADA · Investigala en el grimorio con Tab para comprenderla.');}if(type==='page'&&progress.book&&!progress.spell){offset=0;cipher();}}}
...
function openGrimoire(){
 mode='journal';
 panel(`<div class="eyebrow">EL GRIMORIO DEL ERRANTE</div><div class="grimoire" aria-label="Grimorio abierto con páginas arrancadas"><article class="book-page left-page"><small>MEMORIA DEL CAMINANTE</small><h2>Entre raíces<br>y cenizas</h2><div class="book-seal" aria-hidden="true">ᛟ</div><p>El cuero conserva las marcas del tiempo. Junto al lomo quedan hilos sueltos y bordes de hojas arrancadas.</p>${progress.dash?'<p class="book-note">Botas del viento · Shift</p>':''}<span class="page-number">I</span></article><div class="torn-stubs" aria-hidden="true"><i></i><i></i><i></i><i></i></div><article class="book-page right-page ${progress.page?'restored':'missing'}">${progress.page?progress.spell?'<small>PÁGINA RECUPERADA · TRADUCIDA</small><h2>Ascua</h2><div class="spell-seal" aria-hidden="true">✦</div><blockquote>La llama abre el camino.</blockquote><p>Concentra una chispa de magia antigua y lánzala hacia delante. Quema a tus enemigos y las zarzas que cierran el paso.</p><dl><dt>Invocación</dt><dd>K</dd><dt>Coste</dt><dd>1 carga de magia</dd><dt>Impacto</dt><dd>2 puntos de daño</dd></dl><p class="book-note">Los golpes de espada que aciertan recuperan magia.</p>':'<small>PÁGINA RECUPERADA · SIN INVESTIGAR</small><h2>Página ilegible</h2><p>El fragmento encaja entre los restos del lomo. Su escritura todavía oculta el significado.</p><div class="book-runes">OD OODPD<br>DEUH HO FDPLQR</div><p class="book-note">— LXOLQ VDHUK</p><button id="research" class="primary">INVESTIGAR PÁGINA</button>':'<small>HUELLAS DE UNA AUSENCIA</small><div class="missing-leaf" aria-hidden="true"></div><h2>Hojas arrancadas</h2><p>Alguien se llevó las palabras.<br>Solo quedaron sus bordes.</p>'}<span class="page-number">${progress.page?'II':'—'}</span></article></div><button id="back" class="secondary">CERRAR EL LIBRO</button>`);
 $('#overlay .panel').classList.add('book-panel');
 $('#back').onclick=resume;
 if($('#research'))$('#research').onclick=()=>{offset=0;cipher();$('#back').onclick=openGrimoire;};
}

function openMap(){
 if(mode!=='play')return;
 if(!progress.map){toast('Todavía no tenés un mapa.');return;}
 mode='map';
 const positions={0:[0,1],1:[1,1],2:[2,1],3:[0,0],4:[1,0],5:[2,0]};
 const visible=progress.visited;
 const lines=[[0,1],[1,2],[3,4],[4,5],[0,3],[1,4]].filter(([a,b])=>visible.includes(a)&&visible.includes(b)).map(([a,b])=>{const [ax,ay]=positions[a],[bx,by]=positions[b];return `<line x1="${100+ax*200}" y1="${70+ay*150}" x2="${100+bx*200}" y2="${70+by*150}"/>`;}).join('');
 panel(`<div class="world-map"><small>CARTA DEL CAMINANTE</small><h2>El bosque olvidado</h2><div class="map-chart"><svg viewBox="0 0 600 290" aria-hidden="true">${lines}</svg>${visible.map(n=>{const [x,y]=positions[n];return `<div class="map-room ${room===n?'current':''}" style="left:${(100+x*200)/6}%;top:${(70+y*150)/2.9}%">${rooms[n].name}${room===n?'<strong>ESTÁS AQUÍ</strong>':''}</div>`;}).join('')}</div><p>El camino se dibuja con tus pasos.</p><button id="back" class="primary">M · GUARDAR MAPA</button></div>`);
 $('#overlay .panel').classList.add('map-panel');
 $('#back').onclick=resume;
}
